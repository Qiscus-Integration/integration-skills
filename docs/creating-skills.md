...
- If the skill depends on files in `references/` or `scripts/`, add a `## Bundled Resources` section that lists them and explains when to open them explicitly
...
## Full Folder Structure (for skills with external references)

For complex skills that need additional files (Codex only):
...
> **Note:** `scripts/` and `references/` are only supported by Codex.
> Claude Code does not load these files automatically.
> Add a `## Bundled Resources` section in `SKILL.md` whenever a skill depends on them.
