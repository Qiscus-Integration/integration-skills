...
- The task is only about writing test cases (use `qa-plan`)
- The task is only about UI/UX design decisions (use `design-plan`)
...
[QA] qa-plan reads prd.md + design-spec.md  →  qa-testplan.md
                                                       ↓
[Dev] dev-plan reads all 3 files  →  dev-plan.md  ← YOU ARE HERE
```
...
3. If `prd.md` does not exist, **stop completely**:
   ...
   Ask the PM to run $prd-plan first.
   ```
4. If `design-spec.md` does not exist, **warn and continue**:
   ...
   Ask the UI/UX Designer to run $design-plan for a more complete plan.
   ```
5. If `qa-testplan.md` does not exist, **warn and continue**:
   ...
   Ask the QA Engineer to run $qa-plan for a more complete plan.
   ```
