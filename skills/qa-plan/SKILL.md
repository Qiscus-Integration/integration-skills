...
- No PRD or user stories are available — run `prd-plan` first
...
[PM] prd-plan  →  prd.md
                         ↓
[Designer] design-plan reads prd.md  →  design-spec.md
                                           ↓
[QA] qa-plan reads prd.md + design-spec.md  →  qa-testplan.md  ← YOU ARE HERE
                                                       ↓
[Dev] dev-plan reads all 3 files  →  dev-plan.md
```
...
4. If `prd.md` does not exist, stop and say:
   ...
   Please ask the PM to run $prd-plan first.
   ```
5. If `design-spec.md` does not exist, warn the user:
   ...
   Ask the UI/UX Designer to run $design-plan for more complete test coverage.
   ```
...
- Open `assets/templates/csv-template.md.template` and follow all instructions there.
...
- If `output_format = csv` or `output_format = both`:
  - Open `assets/templates/csv-template.md.template` first.
...
Next step: hand prd.md + design-spec.md + qa-testplan.md to the Developer and ask them to run $dev-plan
```
...
- `assets/templates/csv-template.md.template` — CSV column structure, step format rules, and sample rows for CSV export.
